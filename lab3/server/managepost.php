                ...
                <div id="Extension1">
                  <div class="header">
                    <table>
                      <tbody>
                        <tr>
                          <td style="padding-left:0;width:100%"><button class="button" onclick="window.location='createpost.php'">Đăng tin</button></td>
                  ...
                  <table class='table'>
                    ...
                    $listProduct = $con->query('select Id, ProductName, SalePrice, CategoryName from product');
                    ...
                    while ($row = $listProduct->fetch_assoc()) {
                      echo "
                          <tr>
                              ...
                              <td><a class='button edit' href='editpost.php?product_id=" . $row['Id'] . "&action=edit'>Edit</a></td>
                              <td><a class='button delete-btn' href='postcontroller.php?product_id=" . $row['Id'] . "&action=delete'>Delete</a></td>
                          </tr>";
                    }
  ...
  <script>
    ...
    for (let i = 0; i < deleteButtonList.length; ++i) {
      deleteButtonList[i].addEventListener('click', function(e) {
        e.preventDefault();
        let confirmDelete = confirm('Bạn có chắc chắn muốn xóa tin đăng này không?');
        if (confirmDelete) {
